...
public class CircularLayout : LayoutGroup
{
    ...
    [SerializeField]
    protected int m_Radius = 100;
    public int Radius { get { return m_Radius; } set { SetProperty( ref m_Radius, Mathf.Max( 1, value ) ); } }
    ...
    public override void CalculateLayoutInputHorizontal()
    {
        ...
        //    -1, 0 );
        SetLayoutInputForAxis( Radius, Radius, -1, 0 );
    }
    ...
    public override void CalculateLayoutInputVertical()
    {
        ...
        SetLayoutInputForAxis( Radius, Radius, -1, 1 );
    }
    ...
    private void SetCellsAlongAxis( int axis )
    {
        ...
        Vector2 startOffset = new Vector2(
                GetStartOffset( 0, Radius ),
                GetStartOffset( 1, Radius )
                );
        ...
        for ( int i = 0 ; i < rectChildren.Count ; i++ )
        {
            ...
            float angle = Mathf.LerpAngle( 0.0f, 360 * Mathf.Deg2Rad, ( float )i / rectChildren.Count );

            //SetChildAlongAxis( rectChildren[ i ], 0, startOffset.x + ( cellSize[ 0 ] + spacing[ 0 ] ) * positionX, cellSize[ 0 ] );
            //SetChildAlongAxis( rectChildren[ i ], 1, startOffset.y + ( cellSize[ 1 ] + spacing[ 1 ] ) * positionY, cellSize[ 1 ] );
            SetChildAlongAxis( rectChildren[ i ], 0, startOffset.x + Radius * Mathf.Cos( angle ) );
            SetChildAlongAxis( rectChildren[ i ], 1, startOffset.y + Radius * Mathf.Sin( angle ) );
        }
    }
}
